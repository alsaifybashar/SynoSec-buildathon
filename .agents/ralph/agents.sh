...
AGENT_CODEX_INTERACTIVE_CMD="codex exec --yolo --skip-git-repo-check \"\$(cat {prompt})\""
AGENT_CLAUDE_CMD="claude -p --dangerously-skip-permissions \"\$(cat {prompt})\""
AGENT_CLAUDE_INTERACTIVE_CMD="claude -p --dangerously-skip-permissions \"\$(cat {prompt})\""
AGENT_DROID_CMD="droid exec --skip-permissions-unsafe -f {prompt}"
AGENT_DROID_INTERACTIVE_CMD="droid exec --skip-permissions-unsafe -f {prompt}"
AGENT_OPENCODE_CMD="opencode run \"\$(cat {prompt})\""
AGENT_OPENCODE_INTERACTIVE_CMD="opencode run \"\$(cat {prompt})\""
