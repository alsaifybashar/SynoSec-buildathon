...
if ! output="$(nxc smb "$target" 2>&1)"; then
  escaped_output="$(node -p "JSON.stringify(process.argv[1])" "$output")"
  printf '{"output":%s,"statusReason":"NetExec failed","commandPreview":"nxc smb %s"}\n' "$escaped_output" "$target"
  exit 64
fi

summary="NetExec completed assessment against $target."
escaped_output="$(node -p "JSON.stringify(process.argv[1])" "$output")"
escaped_summary="$(node -p "JSON.stringify(process.argv[1])" "$summary")"
escaped_evidence="$(node -p "JSON.stringify(process.argv[1])" "$output")"
printf '{"output":%s,"observations":[{"key":"netexec:%s","title":"NetExec completed","summary":%s,"severity":"info","confidence":0.7,"evidence":%s,"technique":"NetExec assessment"}],"commandPreview":"nxc smb %s"}\n' "$escaped_output" "$target" "$escaped_summary" "$escaped_evidence" "$target"
