...
if ! output="$(patator  "$target" 2>&1)"; then
  escaped_output="$(node -p "JSON.stringify(process.argv[1])" "$output")"
  printf '{"output":%s,"statusReason":"Patator failed","commandPreview":"patator  %s"}\n' "$escaped_output" "$target"
  exit 64
fi

summary="Patator completed assessment against $target."
escaped_output="$(node -p "JSON.stringify(process.argv[1])" "$output")"
escaped_summary="$(node -p "JSON.stringify(process.argv[1])" "$summary")"
escaped_evidence="$(node -p "JSON.stringify(process.argv[1])" "$output")"
printf '{"output":%s,"observations":[{"key":"patator:%s","title":"Patator completed","summary":%s,"severity":"info","confidence":0.7,"evidence":%s,"technique":"Patator assessment"}],"commandPreview":"patator  %s"}\n' "$escaped_output" "$target" "$escaped_summary" "$escaped_evidence" "$target"
