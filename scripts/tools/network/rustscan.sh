...
if ! output="$(rustscan -a "$target" 2>&1)"; then
  escaped_output="$(node -p "JSON.stringify(process.argv[1])" "$output")"
  printf '{"output":%s,"statusReason":"RustScan failed","commandPreview":"rustscan -a %s"}\n' "$escaped_output" "$target"
  exit 64
fi

summary="RustScan completed assessment against $target."
escaped_output="$(node -p "JSON.stringify(process.argv[1])" "$output")"
escaped_summary="$(node -p "JSON.stringify(process.argv[1])" "$summary")"
escaped_evidence="$(node -p "JSON.stringify(process.argv[1])" "$output")"
printf '{"output":%s,"observations":[{"key":"rustscan:%s","title":"RustScan completed","summary":%s,"severity":"info","confidence":0.7,"evidence":%s,"technique":"RustScan assessment"}],"commandPreview":"rustscan -a %s"}\n' "$escaped_output" "$target" "$escaped_summary" "$escaped_evidence" "$target"
