  ...
  return vi.fn(async (input: RequestInfo | URL) => {
    ...
    if (url.startsWith("/api/targets?")) {
      return new Response(JSON.stringify({
        targets: [seededTarget, fullStackTarget, juiceShopTarget, target],
        page: 1,
        pageSize: 25,
        total: 4,
        totalPages: 1
    ...
    if (url === `/api/targets/${juiceShopTarget.id}`) {
      return new Response(JSON.stringify(juiceShopTarget));
    }
...
describe("TargetsPage", () => {
  ...
  it("shows Juice Shop guidance for the local lab target", async () => {
    vi.stubGlobal("fetch", createFetchMock());
    ...
    expect(await screen.findByRole("tooltip")).toHaveTextContent("Broad OWASP Juice Shop challenge surface");
    expect(screen.getByRole("tooltip")).toHaveTextContent("Pinned official Docker image for controlled local-only testing");
  });
