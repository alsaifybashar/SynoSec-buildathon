...
import { normalizeToolInput, parseExecutionTarget, parseTarget, truncate } from "@/features/workflows/workflow-execution.utils.js";
import { selectToolsForContext, type ToolSelectorContext } from "@/features/scans/runtime/tool-selector.js";
...
export class SingleAgentScanService {
  ...
  async runWorkflowLinkedScan(input: {
    runId: string;
  ...
  private async createExecutionContext(input: {
    ...
    const [application, runtime, agent] = await Promise.all([
      ...
      input.input.runtimeId ? this.runtimesRepository.getById(input.input.runtimeId) : Promise.resolve(null),
      this.aiAgentsRepository.getById(input.input.agentId)
    ]);
