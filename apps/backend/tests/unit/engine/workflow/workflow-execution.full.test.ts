...
describe("WorkflowExecutionService", () => {
  ...
  it("injects pre-run bundle evidence into the first model turn when enabled", async () => {
    streamTextMock.mockImplementationOnce((options: { system: string; tools: Record<string, { execute: (input: unknown) => Promise<unknown> }> }) => ({
      fullStream: (async function* () {
        expect(options.system).toContain("Scan preamble:");
        expect(options.system).toContain("Key findings:");
        expect(options.system).toContain("Routes and content:");
        expect(options.system).toContain("Services and protocols:");
        ...
        expect(options.system).not.toContain("Recommended next action");
        await options.tools.report_system_graph_batch.execute({
          resources: [{
            id: "resource:demo-target",
            kind: "host",
            name: "demo-target"
          }]
        });
    ...
    await vi.waitFor(() => {
      ...
      expect(service.createdRuns[0]?.status).toBe("completed");
    });

    const renderedPromptEvent = service.createdRuns[0]?.events.find((event) => event.title === "Rendered system prompt");
    const initialScanningPromptEvent = service.createdRuns[0]?.events.find((event) => event.title === "Rendered scan preamble prompt section");
    expect(renderedPromptEvent?.detail).not.toContain("Scan preamble:");
    expect(initialScanningPromptEvent?.detail).toContain("Scan preamble:");
    expect(initialScanningPromptEvent?.detail).toContain("Key findings:");
    expect(initialScanningPromptEvent?.payload["promptSectionTitle"]).toBe("Scan preamble");
  });

  it("continues into model execution when some enabled scan-preamble tools fail", async () => {
    streamTextMock.mockImplementationOnce((options: { system: string; tools: Record<string, { execute: (input: unknown) => Promise<unknown> }> }) => ({
      fullStream: (async function* () {
        expect(options.system).toContain("Scan preamble:");
        expect(options.system).toContain("Root page responded.");
        expect(options.system).toContain("Port 80/http is reachable.");
        expect(options.system).not.toContain("Missing required binary: httpx");
        await options.tools.report_system_graph_batch.execute({
          resources: [{
            id: "resource:partial-scan",
            kind: "host",
            name: "partial-scan"
          }]
        });
        await options.tools.complete_run.execute({
          summary: "Continued after partial scan preamble failures."
        });
        yield {
          type: "finish",
          finishReason: "stop",
    ...
    expect(latestTitles).toContain("Scan preamble tool failed: HTTP Recon");
  });

  it("runs the scan preamble even when the workflow tool surface excludes those scan tools", async () => {
    streamTextMock.mockImplementationOnce((options: { system: string; tools: Record<string, { execute: (input: unknown) => Promise<unknown> }> }) => ({
      fullStream: (async function* () {
        expect(options.system).toContain("Scan preamble:");
        expect(options.system).toContain("Supporting excerpts:");
        expect(options.system).toContain("Root page responded.");
        await options.tools.report_system_graph_batch.execute({
          resources: [{
            id: "resource:global-scan",
            kind: "host",
            name: "global-scan"
          }]
        });
        await options.tools.complete_run.execute({
          summary: "Completed after global scan preamble."
        });
        yield {
          type: "finish",
          finishReason: "stop",
    ...
    expect(streamTextMock).toHaveBeenCalled();
    expect(service.createdRuns[0]?.events.some((event) => event.title === "Scan preamble tool completed: HTTP Recon")).toBe(true);
  });
