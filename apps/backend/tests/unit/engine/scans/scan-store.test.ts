...
describe("scan-store vulnerability loading", () => {
  afterEach(() => {
    vi.resetModules();
    vi.unmock("@/shared/database/prisma-client.js");
  });

  it("fails loudly when a stored vulnerability row is malformed", async () => {
    vi.doMock("@/shared/database/prisma-client.js", () => ({
      prisma: {
        scanFinding: {
    ...
    const { getSecurityVulnerabilitiesForScan } = await import("@/engine/scans/scan-store.js");
