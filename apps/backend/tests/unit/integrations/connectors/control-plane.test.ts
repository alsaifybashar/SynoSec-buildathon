...
describe("connectorControlPlane", () => {
  ...
  it("advertises the native auth probe when connector policy allows native http actions", () => {
    connectorControlPlane.register({
    ...
    const status = connectorControlPlane.getStatus();
    expect(status.connectors[0]?.supportedToolIds).toEqual(expect.arrayContaining([
