...
import { buildWorkflowTranscript, getWorkflowRunContextTokenEstimate, getWorkflowRunModelStepCount, getWorkflowRunTokenUsage } from "../../src/workflow-presentation.js";
...
describe("buildWorkflowTranscript", () => {
  ...
  it("projects configurable pre-scanning as scan-preamble system messages instead of tool details", () => {
    const run: WorkflowRun = {
      ...
      events: [
        {
          ...
          type: "system_message",
          status: "completed",
          title: "Scan preamble started",
          summary: "Running 1 automatic scan tool before the first model turn.",
        ...
        },
        {
          ...
          type: "tool_result",
          status: "completed",
          title: "Scan preamble tool completed: Nmap Scan",
    ...
    const initialScanningItems = transcript.items.filter((item) => item.kind === "system_message" && item.title === "Scan preamble");
  ...
  it("splits prompt context into structured sections including scan-preamble prompt context", () => {
    const run: WorkflowRun = {
      id: "50000000-0000-0000-0000-000000000002c",
      workflowId: workflow.id,
      workflowLaunchId: "60000000-0000-0000-0000-000000000002c",
      targetId: "70000000-0000-0000-0000-000000000002c",
      preRunEvidenceEnabled: true,
      preRunEvidenceOverride: null,
      status: "completed",
      currentStepIndex: 0,
      ...
      events: [
        {
          id: "prompt-system",
          workflowRunId: "50000000-0000-0000-0000-000000000002c",
          workflowId: workflow.id,
          workflowStageId: null,
          stepIndex: 0,
          ord: 0,
          ...
          summary: "Persisted the workflow-owned system prompt section delivered to the workflow model.",
          detail: "Base prompt body",
          payload: {
            messageKind: "prompt",
            promptKind: "system",
            body: "Base prompt body"
          },
          createdAt: "2026-04-25T00:00:00.010Z"
        },
        {
          id: "prompt-prerun",
          workflowRunId: "50000000-0000-0000-0000-000000000002c",
          ...
          detail: "Scan preamble:\n- Observation: Ports 80 and 443 are reachable.",
          payload: {
            messageKind: "prompt",
            promptKind: "system",
          ...
          createdAt: "2026-04-25T00:00:00.020Z"
        }
      ]
    };
    ...
    const promptContext = transcript.items.find((item) => item.kind === "system_message" && item.title === "Prompt context");
    expect(promptContext && promptContext.kind === "system_message" ? promptContext.sections.map((section) => section.title) : []).toEqual([
      "System prompt",
      "Scan preamble"
    ]);
  });
